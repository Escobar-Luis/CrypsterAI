...
function CryptoCard({ crypto, setOpen, setmore }) {
  const [hover, setHover] = useState(false);
  function GetShownData() {
    fetch(
      `https://api.coingecko.com/api/v3/coins/${crypto.id}?localization=false&tickers=false&market_data=true&community_data=true&developer_data=false`
    )
      .then((r) => r.json())
      .then((d) => {
        setmore(d);
    ...
    >
      ...
      >
        <img
          ...
          className={
            hover
              ? "  brightness-125 mx-auto col-span-2 h-20 w-20 mt-3 p-1 mb-3 rounded border bg-inherit border-white shadow-2xl animate-bounce"
              : " mx-auto col-span-2 h-20 w-20 mt-3 p-1 mb-3 rounded border bg-inherit border-white shadow-2xl animate-none "
          }
        />
        <h1
          className={
            crypto.id.length > 12
              ? " underline underline-offset-2 decoration-wavy text-left col-span-3 text-sm font-semibold mb-3"
              : " mb-3 underline underline-offset-2 decoration-wavy text-left col-span-3 text-xl font-semibold"
          }
        >
        ...
        <p
          className={
            crypto.current_price.length > 9
              ? "text-sm mb-3 font-bold"
              : "text-lg mb-3 font-bold"
          }
        >
          <span className="text-lg text-slate-900 ">
            PRICE: <span className="text-green-400">$</span>
          </span>{" "}
        ...
        <p
          className={
            hover
              ? " border-4 border-double ml-auto font-bold text-xl  bg-gradient-to-r from-slate-300 to-transparent h-fit w-fit p-2 rounded-full mb-3 shadow-lg shadow-orange-300"
              : "border-4 border-double ml-auto font-bold text-xl  bg-gradient-to-r from-slate-300 to-transparent h-fit w-fit p-2 rounded-full mb-3 shadow-lg shadow-slate-500"
          }
        >
          {crypto.symbol.toUpperCase()}
        </p>
        {hover ? (
          <h1 className="font-bold animate-pulse text-center border-2 border-black border-dotted  bg-gradient-to-r from-violet-500 via-slate-300 to-violet-500 rounded-full p-1 ">
            More Info?
          </h1>
        ) : null}
      </div>
