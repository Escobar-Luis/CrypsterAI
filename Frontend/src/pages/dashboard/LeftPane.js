...
function LeftPane({ s, a, setshown, shown, brolic, click, results, seen, handleSelectedResult, chartForm }) {
      ...
      <input
        ...
        {a!== null? a.map((c) => {
         return visibleCryptos?.map((pc) => {
            ...
            else if (click===false && seen==='smac') {
          
              return <div className="res w-full  h-[35rem]   ">
                  <Table chartForm={chartForm} handleSelectedResult={handleSelectedResult} results={results}/>
        {/* {
        a!== null? a.map((c) => {
        return (
          <div
